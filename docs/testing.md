...
| セッション復元 | Cognito にユーザーなし | トークンが設定されない、`isInitialized` が `true` |
| セッション復元 | セッション有効 | トークンが設定される、`isInitialized` が `true` |
| セッション復元 | セッション無効 | トークンが設定されない |
| セッション復元 | `getSession` がエラー返却 | トークンが設定されない |
...
| `forgotPassword` | 送信成功 | `success: true` を返す |
| `forgotPassword` | 送信失敗 | `success: false`、エラーメッセージを返す |
| `confirmForgotPassword` | リセット成功 | `success: true` を返す |
| `confirmForgotPassword` | リセット失敗 | `success: false`、エラーメッセージを返す |
| `resendConfirmationCode` | 再送成功 | `success: true` を返す |
| `resendConfirmationCode` | 再送失敗 | `success: false`、エラーメッセージを返す |
| `getFreshToken` | Cognito ユーザーなし | `null` を返す |
| `getFreshToken` | セッション有効 | トークンを返し `tokenSignal` も更新する |
| `getFreshToken` | セッション無効 | `null` を返し `tokenSignal` をクリアする |
| `getFreshToken` | `getSession` がエラー返却 | `null` を返し `tokenSignal` をクリアする |
...
| `getRegistrationTargets` | API エラー | エラーを伝播する |
| `getRemainingRaces` | 正常 | 残レース情報の一覧を取得する |
| `getRemainingRaces` | API エラー | エラーを伝播する |
| `getPatterns` | 正常 | 指定ウマ娘のパターンを取得する |
| `getPatterns` | API エラー | エラーを伝播する |
| `registerBatchResults` | 正常 | 全レースを一括登録する |
| `registerBatchResults` | API エラー | エラーを伝播する |
...
| `showSessionExpired` — 通常状態 | `persistent: true`、`type: 'error'` のトーストが表示される |
| `showSessionExpired` — persistent 表示中 | 早期リターンし状態が変わらない（重複防止） |
| `showSessionExpired` — 通常トースト表示中 | persistent トーストで上書きする |
| `showSessionExpired` — タイマーキャンセル | `show()` のタイマーをクリアし、3 秒後も表示され続ける |
...
| `getRunSeason` | レースフラグ（`junior_flag` 等）の組み合わせ（単独/複合/全フラグ）からシーズン文字列を生成する |
